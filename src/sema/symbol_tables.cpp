...
//    linkage_symbol = lookup(token, SymbolKind::OBJECT);  // object and function follow the same rule
//    if (linkage_symbol) {
//      // type check
...
//  }
//}
const ISymbol *SymbolTable::lookup(const Token &token,
                                   SymbolKind symbol_kind) const {
  try {
    const ISymbol *symbol = at(token.getValue()).get();
    if (symbol->getKind() != symbol_kind) return nullptr;
    else return symbol;
  } catch (const std::out_of_range &) {
    if (father) {
      return father->lookup(token, symbol_kind);
    } else {
      return nullptr;
    }
  }
...
bool SymbolTable::isTypedef(const Token &token) {
  return lookup(token, SymbolKind::TYPEDEF) != nullptr;
}
const ISymbol *SymbolTable::insert(const Token &token, std::unique_ptr<ISymbol> &&symbol) {
  try {
    at(token.getValue());
